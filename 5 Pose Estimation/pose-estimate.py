...
#kpts:0:nose,12,13:hips content in kpts:(x,y,confidence)
#poses:
#pose[2] and pose[3] represent the x and y coordinates of the center of the bounding box.
#pose[4] and pose[5] represent the width and height of the bounding box.
#kpts value from the 6th element
#0.0000001 is aiming to avoid the situation of /0




def run(poseweights="yolov7-w6-pose.pt",source="0",device='cpu',view_img=True,blur = False,
                ...
                for i, pose in enumerate(output_data):  # detections per image
                    ...
                    if len(output_data):  #check if no pose
                        ...
                        for det_index, (*xyxy, conf, cls) in enumerate(reversed(pose[:,:6])): #loop over poses for drawing on frame
                            ...
                            plot_one_box_kpt(xyxy, im0, pose, label=label, color=colors(c, True), 
                                        line_thickness=opt.line_thickness,kpt_label=True, kpts=kpts, steps=3, 
                                        orig_shape=im0.shape[:2],fall_detect= True, blur = blur )
